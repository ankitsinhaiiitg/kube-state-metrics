...
var (
	...
	persistentVolumeClaimMetricFamilies = []metric.FamilyGenerator{
		...
		{
			Name: "kube_persistentvolumeclaim_resized_status_condition",
			Help: "Information about the status of resizing of persistent volume claim.",
			Type: metric.Gauge,
			GenerateFunc: wrapPersistentVolumeClaimFunc(func(p *v1.PersistentVolumeClaim) *metric.Family {
				...
				for i, c := range p.Status.Conditions {
					...
					for j, m := range conditionMetrics {
						...
						metric.LabelKeys = []string{"type", "status"}
